...
Wrapper script for performing vulnerability scan or image analysis on local images, utilizing the Sysdig inline_scan container.
...
Images should be built & tagged locally.
  Usage: ${0##*/} analyze -k <API Token> [ OPTIONS ] <FULL_IMAGE_TAG>
    ...
    -m <PATH>  [optional] Path to image manifest (ex: -m ./manifest.json)
...
get_and_validate_analyzer_options() {
  ...
  if [[ ! $(which podman) ]]; then
      printf '\n\t%s\n\n' 'ERROR - podman is not installed or cannot be found in $PATH' >&2
      ...
      display_usage_analyzer >&2
      exit 1
  elif [[ "${#@}" -lt 1 ]] && [[ -z "${IMAGE_TO_SCAN}" ]]; then
...
get_and_validate_images() {
  ...
  if [[ "${#FAILED_IMAGES[@]}" -gt 0 ]]; then
    ...
    if [[ "${#FAILED_IMAGES[@]}" -ge "${#IMAGE_NAMES[@]}" ]]; then
        printf '\n\t%s\n\n' "ERROR - no local images specified in script input: ${0##*/} ${IMAGE_NAMES[*]}" >&2
        display_usage >&2
        exit 1
    fi
...
start_analysis() {
  ...
  if [[ ! "${i_flag-""}" ]]; then
      SYSDIG_IMAGE_ID=$(podman inspect --format "{{.Id}}" "$i" | cut -f2 -d ":" )
  fi
  ...
  if [[ "${FULLTAG}" =~ "@sha256:" ]]; then
      local repoTag=$(podman inspect --format '{{ if .RepoTags }}{{ index .RepoTags 0 }}{{ else }}{{ end }}' ${SCAN_IMAGES[0]} | cut -f 2 -d ":")
  ...
  FULL_IMAGE_NAME=$(podman inspect --format '{{if .RepoDigests }}{{index .RepoDigests 0}}{{ else }}{{ end }}' ${SCAN_IMAGES[0]} | cut -d "@" -f 1)
...
post_analysis() {
  ...
  printf '%s\n' " Retreive results from scanning engine."
  podman cp "${DOCKER_NAME}:/anchore-engine/image-analysis-archive.tgz" "${TMP_PATH}/image-analysis-archive.tgz"
...
get_repo_digest_id() {
  ...
  DIGESTS=$(podman inspect --format '{{.Digest}}' "${SCAN_IMAGES[0]}")
  ...
  if [[ -z "${REPO_DIGEST:-}" ]]; then
      printf '%s\n' " Unable to compute the digest from podman inspect ${SCAN_IMAGES[0]}"
      printf '%s\n' " Consider running with -d option with a valid sha256:<digestID>."
      SYSDIG_IMAGE_DIGEST=$(podman inspect "${SCAN_IMAGES[0]}" | ${SHASUM_COMMAND} | awk '{ print $1 }' | tr -d "\n")
      SYSDIG_IMAGE_DIGEST=$(echo "sha256:${SYSDIG_IMAGE_DIGEST}")
  else
      # Use parsed digest from array of digests based on podman inspect result
...
save_and_copy_images() {
  local base_image_name=$(echo ${BASE_IMAGE}-${TAG} | rev | cut -d '/' -f 1 | rev )
  ...
  if [[ ! "${image_name}" =~ [:]+ ]]; then
      podman save "${SCAN_IMAGES[0]}:latest" -o "${save_file_path}"
  else
      podman save "${SCAN_IMAGES[0]}" -o "${save_file_path}"
  fi

  # hack to fix file name issue
  new_file_name="${BASE_IMAGE}:${TAG}.tar"
  new_file_path="${TMP_PATH}/${new_file_name}"

  mv "${save_file_path}" "${new_file_path}"

  save_file_name=${new_file_name}
  save_file_path=${new_file_path}
  ...
  echo
  printf '%s\n' " Copy files to local scanning engine."
